...
install_neovim() {
  echo -e "${GREEN}Installing Neovim ${VERSION} version${ENDCOLOR}."
  sleep 2

  # Set download URL based on version
  if [[ "$VERSION" == "stable" ]]; then
    echo -e "${YELLOW}Downloading stable version${ENDCOLOR}."
    curl -LO https://github.com/neovim/neovim/releases/download/stable/nvim-linux-x86_64.tar.gz
  else
    echo -e "${YELLOW}Downloading nightly version${ENDCOLOR}."
