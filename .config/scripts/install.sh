...
detect_system() {
  ...
  if command -v apt >/dev/null 2>&1; then
    ...
    if command -v yay >/dev/null 2>&1; then
      ...
      echo -e "${YELLOW}No AUR helper found. AUR packages will be skipped.${ENDCOLOR}"
      AUR_HELPER=""
      AUR_INSTALL_CMD=""
    fi
...
# Helper functions to check if a packages is installed
package_installed() {
  local package="$1"
  case "$PKG_MANAGER" in
    apt)
...
aur_package_installed() {
  ...
  if [ -n "$AUR_HELPER" ]; then
    "$AUR_HELPER" -Qi "$package" >/dev/null 2>&1
  else
    return 1
  fi
}
      ...
      if eval "$INSTALL_CMD" "${packages[@]}"; then
        ...
        if [ ${#failed_packages[@]} -gt 0 ]; then
          ...
          for package in "${failed_packages[@]}"; do
            ...
            if eval "$INSTALL_CMD" "$package" && package_installed "$package"; then
              echo -e "${GREEN}Successfully installed $package${ENDCOLOR}"
            else
          ...
          # Log all failures at once after retries are done
          if [ ${#still_failed[@]} -gt 0 ]; then
            printf '%s\n' "${still_failed[@]}" >> ~/failed_base_packages.txt
            echo
            echo -e "${RED}Failed packages logged to ~/failed_base_packages.txt: ${still_failed[*]}${ENDCOLOR}"
          fi
        fi
      fi
    else
...
install_aur_packages() {
  if [ "$PKG_MANAGER" != "pacman" ]; then
    echo -e "${YELLOW}AUR packages are only available on Arch Linux. Skipping...${ENDCOLOR}"
    return
  fi

  if [ -z "$AUR_HELPER" ]; then
    echo -e "${YELLOW}No AUR helper found. Please install yay or paru to install AUR packages.${ENDCOLOR}"
    return
  fi

  read -rp "${GREEN}Would you like to install AUR packages? (Yes/No)${ENDCOLOR}: " choice < /dev/tty
  if [[ "$choice" == [Yy]* ]]; then
    echo -e "${GREEN}Installing AUR packages...${ENDCOLOR}"
    ...
    for package in "${AUR_PACKAGES[@]}"; do
      ...
      echo -e "${GREEN}Installing AUR package: $package${ENDCOLOR}"
      if ! eval "$AUR_INSTALL_CMD" "$package"; then
        echo -e "${RED}Failed to install AUR package: $package${ENDCOLOR}"
        echo "$package" >> ~/AUR_packages.txt
        echo
        echo -e "${RED}For a list of failed packages, see ~/AUR_packages.txt"
      fi
    done

    echo -e "${GREEN}Finished installing AUR packages${ENDCOLOR}"
  else
    echo -e "${YELLOW}Skipping AUR packages installation${ENDCOLOR}"
  fi
}
